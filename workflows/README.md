...
`n8n/` 存放可导入 n8n 的 visual workflow assets。当前 n8n 示例映射商品/品牌资料到完整视频的生产链路，并通过创建 run + 每个 Agent 节点单独 HTTP 调用的方式触发 Planner、Story、Script、Director、Visual Director、Asset、Image、Video、Editor 闭环；Tavern 后端 `WorkflowDefinition` 仍是 source of truth。
